...
def plotReprojectionErrorAcc():
    ...
    for v1,v2 in rotationMatrix.matched_vectors_linearfit_acc:
        ...
        repro_err_mag_acc.append(np.linalg.norm(v2 - v1) / np.linalg.norm(v2))
        repro_err_ang_acc.append(utility.angle_between(v1, v2) * 180 / np.pi)
        repro_err_x_acc.append( (v2[0] - v1[0]) / v2[0] )
        repro_err_y_acc.append( (v2[1] - v1[1]) / v2[1] )
        repro_err_z_acc.append( (v2[2] - v1[2]) / v2[2] )
    ...
    plot_repro_err_acc.suptitle( "Reprojection Error Graphs for ACC Data" )
    plt.subplot(511)
...
def plotReprojectionErrorGyro():
    ...
    for v1,v2 in rotationMatrix.matched_vectors_linearfit_gyro:
        ...
        repro_err_mag_gyro.append(np.linalg.norm(v2 - v1) / np.linalg.norm(v2) )
        repro_err_ang_gyro.append(utility.angle_between(v1, v2) * 180 / np.pi)
        repro_err_x_gyro.append( (v2[0] - v1[0]) / v2[0] )
        repro_err_y_gyro.append( (v2[1] - v1[1]) / v2[1] )
        repro_err_z_gyro.append( (v2[2] - v1[2]) / v2[2] )
    ...
    plot_repro_err_gyro.suptitle( "Reprojection Error Graphs for GYRO Data" )
    plt.subplot(511)
