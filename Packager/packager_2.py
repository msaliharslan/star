...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 18 23:35:47 2021
...
def generateRgbPackageMatrix(mapMatrix, imgRgb, imgLeft, imgRight):
    ...
    rgbPackageMatrix = np.zeros((imgRgb.shape[0],imgRgb.shape[1], 10)) # r,g,b, fd, depth, fleft, leftIntensity, fright, rightIntensity, point_index
...
def visualizeRgbPackageMatrix(rgbPackageMatrix, path): # r,g,b, fd, depth, fleft, leftIntensity, fright, rightIntensity
    ...
    package_file = open( path + "rgbPackage/rgbPackage.npy", "wb")
    np.save(package_file, rgbPackageMatrix)
    package_file.close()
...
def generateLeftPackageMatrix(mapMatrix, imgRgb, imgLeft, imgRight):
    ...
    leftPackageMatrix = np.zeros((imgLeft.shape[0],imgLeft.shape[1], 10)) # leftIntensity, fd, depth, frgb, r, g, b, fright, rightIntensity
    index_arr = np.arange(0, mapMatrix.shape[0], dtype=np.uint32)
...
def visualizeLeftPackageMatrix(leftPackageMatrix, path):
    ...
    global u_left, u_right, u_rgb
    # Save Package matrix itself first
    package_file = open( path + "leftPackage/leftPackage.npy", "wb")
...
def generateRightPackageMatrix(mapMatrix, imgRgb, imgLeft, imgRight):
    ...
    rightPackageMatrix = np.zeros((imgRight.shape[0],imgRight.shape[1], 10)) # rightIntensity, fd, depth, frgb, r, g, b, fleft, leftIntensity
    index_arr = np.arange(0, mapMatrix.shape[0], dtype=np.uint32)
...
def visualizeRightPackageMatrix(rightPackageMatrix, path):
    ...
    global u_left, u_right, u_rgb
    # Save Package matrix itself first
    package_file = open( path + "rightPackage/rightPackage.npy", "wb")
...
def generateMapMatrix(worldPs_rgb, imgPs_rgb, rgbShape, worldPs_left, imgPs_left, leftShape, worldPs_right, imgPs_right, rightShape):
    ...
    # index_arr = np.arange(0, worldPs_rgb.shape[0], dtype=np.uint32)
    
    mapMatrix = np.zeros((worldPs_rgb.shape[0], 12), dtype=np.int32) # frgb, xrgb, yrgb, rgbDepth, rgb_depth_index,  fleft, xleft, yleft, leftDepth, left_depth_index,  fright, xright, yright, rightDepth, right_depth_index,
    ...
    # # Depth indeces in map matrix
    # mapMatrix[indexRgb[si_wp_rgb],4] = index_arr[indexRgb[si_wp_rgb]]
    # mapMatrix[indexLeft[si_wp_left],9] = index_arr[indexLeft[si_wp_left]]
    # mapMatrix[indexRight[si_wp_right],14] = index_arr[indexRight[si_wp_right]] 
